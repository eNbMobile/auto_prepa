...
### Déployer depuis le dashboard, sans wrangler

Alternative sans rien installer : dash.cloudflare.com → Workers & Pages →
Create → Worker, le nommer `skyjo`, coller le contenu de `src/index.js` dans
l'éditeur et déployer. Puis Settings → Bindings → Add → KV namespace, avec le
nom de variable `PARTIES` et le namespace `skyjo-parties`.
